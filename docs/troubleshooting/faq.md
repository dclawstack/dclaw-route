...
### How do I update DClaw Route?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp route --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Route without Kubernetes?

Yes, for local development:

```bash
cd dclaw-route/frontend && npm run dev
cd dclaw-route/backend && uvicorn dclaw_route.main:app --reload
```
